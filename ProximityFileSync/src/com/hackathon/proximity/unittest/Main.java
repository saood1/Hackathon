...
class MyFirstProgram {
	...
	static void UnitTest1() {// raju-saood-manish
		...
		System.out.println("The source user: "
				+ src.getUserId()
				+ "   Destination user: "
				+ dest.getUserId()
				+ " Direct distance:  "
				+ Utility.distance(dest.getClient().getLocation()
						.getLattitude(), dest.getClient().getLocation()
						.getLongitude(), src.getClient().getLocation()
						.getLattitude(), src.getClient().getLocation()
						.getLongitude())
				+ " \nProximity user calculated by algorithm: "
				+ user.getUserId()
				+ "  Actual geo distance: "
				+ Utility.distance(dest.getClient().getLocation()
						.getLattitude(), dest.getClient().getLocation()
						.getLongitude(), user.getClient().getLocation()
						.getLattitude(), user.getClient().getLocation()
						.getLongitude()));

	}

	static void UnitTest2() {// manish-raju-saood
		ProximityManager proximityManager = ProximityManager.getInstance();
	...
	static void UnitTest3() {// raju-manish-saood
		ProximityManager proximityManager = ProximityManager.getInstance();
		// proximityManager.clear();
		User user1 = CreateUser(5, 4, 0, "raju");
	...
	static void UnitTest4() {// raju-saood-manish
		ProximityManager proximityManager = ProximityManager.getInstance();
		User user1 = CreateUser(5, 4, 0, "raju");
		User user2 = CreateUser(7, 15, 0, "manish");
		User user3 = CreateUser(11, 11, 0, "saood");
	...
	static void UnitTest5() {// raju-saood-manish
		ProximityManager proximityManager = ProximityManager.getInstance();
	...
	public static User CreateUser(int checksum, float x, float y, String userId) {
		...
		User user = Utility.createUser(Utility.createClient(
				Utility.createGeoLocation(x, y, "", ""), "20", 4444), userId,
				list);
		for (int i = checksum; i < checksum + 3; i++) {
			user.addFileMetaData(Utility.createUserFileMetaData(
					Integer.toString(i), 1, "test" + Integer.toString(i)));
		}
